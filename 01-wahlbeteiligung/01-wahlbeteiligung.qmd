...
p_anim <- p +
  transition_states(wahl, wrap = FALSE) +
  shadow_mark(past = TRUE, future = FALSE, 
...
n_frames <- length(unique(df_wbt_total_ab1972$wahl)) * 5 + 8
animate(p_anim, fps = 10, nframes = n_frames, end_pause = 8, rewind = FALSE,
       res = 150, width = 960, height = 800, units = "px", renderer = av_renderer())
anim_save(
  here("01-wahlbeteiligung", "plots", "wbt-altersgruppen-geschlecht.mp4")) 
```
