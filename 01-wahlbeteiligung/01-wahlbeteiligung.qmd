...
df_wbt_total_ab1972 |> 
  ...
  labs(
    ...
    fill = "Geschlecht",
    caption = "Daten: Bundeswahlleiterin, repräsentative Wahlstatistik. Visualisierung: Ansgar Wolsing"
  )
...
df_wbt_total_ab1972 |> 
  ...
  coord_cartesian(
    clip = "off", ylim = c(50, 100)) +
  guides(size = guide_legend(override.aes = list("color" = "grey10", stroke = 0.2))) +
  labs(
    subtitle = "Wahlbeteiligung nach Altersgruppe und Geschlecht",
    x = "Altersgruppe",
    y = "Wahlbeteiligung (in %)",
...
```{r wbt-altersgruppen-animation, gganimate = list(nframes = 100, fps = 12), fig.width=5, fig.height=5, fig.dpi=150, cache=TRUE}
p <- df_wbt_total_ab1972 |> 
  ...
  geom_text(
    ...
    aes(x = altersgruppe, 
        y = wahlbeteiligung,
        label = wahl), 
    color = color_pal[1], hjust = 0, nudge_x = 0.1, family = "Roboto Slab", size = 5,
    fontface = "bold"
  ) +
  scale_x_discrete(position = "top", expand = expansion(add = c(0.3, 0.8))) +
  scale_y_continuous(labels = function(x) paste(x, "%")) +
...
p <- df_wbt_total_ab1972 |> 
  ...
  geom_text(
    ...
    hjust = 0, nudge_x = 0.1, family = "Roboto Slab", size = 3,
    fontface = "bold"
  ) +
  geom_richtext(
    data = ~subset(., altersgruppe == max(altersgruppe)) |> 
    ...
    color = "grey50", size = 12, hjust = 1, family = "Roboto Slab",
...
# Daten für die Label am ersten Balken
df_plot_wbt_altersgrp_labels <- df_plot_wbt_altersgrp |> 
  filter(wahl ==  first(wahl)) |> 
...
df_plot_wbt_altersgrp |> 
  ...
  geom_col(position = "fill", col = "white", linewidth = 0.2) +
  # Label für den ersten Balken
  geom_text(
    data = df_plot_wbt_altersgrp_labels,
    aes(wahl, ypos_label, label = str_wrap(altersgruppe, 5),
        color = ifelse(altersgruppe_highlighted == "3", "white", "black")
        ), 
    inherit.aes = FALSE, size = 2, vjust = 0.5,  family = "Roboto Slab", lineheight = 0.9) +
