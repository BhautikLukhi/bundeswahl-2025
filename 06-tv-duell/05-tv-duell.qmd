  ...
  labs(
    ...
    caption = paste(
      paste(
       ...
       vor der Bundestagswahl auf einer Skala von -5 bis +5 (2025: Politbarometer Februar I)",
      "Daten: Politbarometer (via GESIS & forschungsgruppe.de).
        Visualisierung: Ansgar Wolsing & Thorsten Faas", sep = "<br><br>"
    )),
    x = NULL, y = NULL # "Bewertung"
...
```{r plot-skalo-6, fig.width = 8, fig.height=8}
...
df_skalo |> 
  filter(!(wahl == 2021 & kandidat == "Merkel")) |> 
  mutate(wahl_fct = factor(wahl)) |> 
  ggplot(aes(bewertung, wahl_fct, fill = partei)) +
  geom_hline(
    aes(yintercept = wahl_fct), 
    linewidth = 8, color = alpha("grey80", 0.3)) +
  geom_vline(aes(xintercept = 0), linewidth = 0.5) +
  # geom_line(
  #  aes(group = kandidat),
  #  linetype = "dashed", linewidth = 0.3, col = "grey50"
  # ) +
  geom_point(
    aes(
      col = ifelse(kandidat == "Merkel" & wahl == "2021", "black", "white")),
    size = annotation_size, shape = 21) +
  geom_richtext(
    data = df_kandidaten_pro_btw,
    aes(x = 3.2, y = wahl_fct,
        label = kandidaten_label,
        vjust = 0.5
        ),
    inherit.aes = FALSE,
    family = "Roboto Condensed", size = 4, hjust = 0, # vjust = 0.5, 
    lineheight = 0.85, nudge_x = 0, fill = NA, label.size = 0, angle = 0
  ) +
  # annotate(
  #   "richtext",
  #   y = c("1987"), 
  ...
  theme(
    ...
    axis.text.x.top = element_text(family = "Roboto Condensed SemiBold", size = 14),
    plot.title = element_markdown(size = 18),
    plot.subtitle = element_textbox(size = 14),
    plot.caption = element_textbox(size = 11, margin = margin(t = 8, b = 6)),
    plot.margin = margin(t = 6, r = 60, b = 6, l = 6)
  )
```
