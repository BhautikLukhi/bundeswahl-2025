...
df_bundesrat_bundesregierung <- df_bundesrat_long |> 
  ...
  mutate(
    ...
    koalition_2aus3 = if (length(bund_parteien) >= 3) {
  ...
  ungroup() |> 
  mutate(koalition_gleich_oder_alleinreg = koalition_gleich | koalition_allein)
```
...
df_uebereinstimmungen |> 
  ...
  # Markierung Bundesregierungen
  # geom_segment(
  #   data = df_annotation_bundesregierung,
  #   aes(x = as_datetime(bund_start), xend = as_datetime(bund_start), y = 0.65, yend = 1),
  #   inherit.aes = FALSE,
  #   color = color_pal[1], linetype = "dotted"
  # ) +
  geom_rect(
    data = df_annotation_bundesregierung |> 
  ...
  geom_text(
    ...
    aes(x = as_datetime(bund_start),
        y = 0.98, label = str_wrap(bund_koalition, 5)),
    inherit.aes = FALSE,
    color = "grey30", size = 3.5, hjust = 0, vjust = 1, nudge_x = 100 * 86400,
  ...
  scale_y_continuous(
    breaks = seq(0, 1, 0.1), 
    labels = function(x) ifelse(x == 0.5, "<b style='color:#c17dfa'>50%</b>", scales::percent(x)),
    expand = expansion(mult = 0)) +
  scale_fill_manual(values = c("transparent", "#D9D9D933")) +
  coord_cartesian(ylim = c(0, 1), clip = "off") +
  labs(
    title = "",
    caption = "Kriterium: Koalition auf Landesebene stimmt mit Bund überein oder  Alleinregierung eines der Partner im Bund",
    x = NULL, y = "Anteil der Stimmen (%)"
  ) +
  theme(
