...
df_wk_abstand_area |> 
  ...
  guides(
    # fill = guide_legend(nrow = 1, override.aes = list(size = 5), reverse = TRUE),
    fill = "none",
...
#### Einzelne Centroids verschieben

```{r move-centroids-1}

# library(cartogram)
# 
# shp_wk_centroids_berlin <- shp_wk_centroids |> 
#   filter(LAND_NR == "11" | WKR_NR == "61") |>  # include Potsdam
#   mutate(dummy_weight = 10) |> 
#   cartogram_dorling(weight = "dummy_weight", k = 12, m_weight = 1) |> 
#   st_buffer(dist = 100)
# 
# max(st_area(shp_wk_centroids_berlin$geometry))
```

```{r move-centroids-2}

# shp_wk_centroids_hh <- shp_wk_centroids |> 
#   filter(LAND_NR == "02") |>  
#   mutate(dummy_weight = 10) |> 
#   cartogram_dorling(weight = "dummy_weight", k = 18, m_weight = 1) |> 
#   st_buffer(dist = 2500)
# 
# max(st_area(shp_wk_centroids_hh$geometry))
```

```{r move-centroids-3}

# shp_wk_centroids_koeln <- shp_wk_centroids |> 
#   filter(WKR_NR %in% c("92", "93", "94", "100")) |>  
#   mutate(dummy_weight = 10) |> 
#   cartogram_dorling(weight = "dummy_weight", k = 20, m_weight = 0.6)  |> 
#   st_buffer(dist = 5500)
# 
# max(st_area(shp_wk_centroids_koeln$geometry))
```

```{r move-centroids-4}

# shp_wk_centroids_muc <- shp_wk_centroids |> 
#   filter(WKR_NR %in% c("216", "217", "218", "219")) |>  
#   mutate(dummy_weight = 10) |> 
#   cartogram_dorling(weight = "dummy_weight", k = 25, m_weight = 1)  |> 
#   st_buffer(dist = 4500)
# 
# max(st_area(shp_wk_centroids_muc$geometry))
...
# shp_wk_centroids_shifted <- shp_wk_centroids |> 
#   filter(LAND_NR != "11" & WKR_NR != "61" 
#          & LAND_NR != "02" & 
#            !WKR_NR %in% c("92", "93", "94", "100") &
#            !WKR_NR %in% c("216", "217", "218", "219")
#            ) |> 
#   bind_rows(shp_wk_centroids_berlin) |>
#   bind_rows(shp_wk_centroids_hh) |> 
#   bind_rows(shp_wk_centroids_koeln) |> 
#   bind_rows(shp_wk_centroids_muc) |> 
#   # Wieder alte Sortierung herstellen
#   arrange(WKR_NR) |> 
#   mutate(wk_gewinner_partei = if_else(wk_gewinner_partei %in% c("CDU", "CSU"),
#                                    "CDU/CSU", wk_gewinner_partei),
#        wk_gewinner_partei = factor(wk_gewinner_partei, 
